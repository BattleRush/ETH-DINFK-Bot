...
namespace ETHDINFKBot.Helpers
{
    public class DrawDbSchema : IDisposable
    {
        ...
        public void DrawAllTables()
        {
            ...
            Font drawFont2 = new Font("Arial", 20);
            ...
            int perTableHeight = 400;

            var key = new Bitmap(Path.Combine("Images" ,"keyicon.png"));
            ...
            foreach (var dbTable in DBTableInfo)
            {
                ...
                Graphics.DrawString(dbTable.TableName, drawFont2, b, new Point(leftPadding + (width + leftPadding) * count - 7, topPadding + countTable * height + custHeight - 40));
                ...
                foreach (var field in dbTable.FieldInfos)
                {
                    if (field.IsPrimaryKey)
                    {
                        ...
                        primaryKeys.Add(dbTable.TableName, new Point(x, y));
                    }
                    ...
                    Graphics.DrawString(field.Name, drawFont, b, new Point(leftPadding + (width + leftPadding) * count + 10, topPadding + countTable * height + custHeight + 10));
            ...
            foreach (var dbTable in DBTableInfo)
            {
                ...
                foreach (var field in dbTable.FieldInfos)
                {
                    ...
                    if (field.IsForeignKey && !field.IsPrimaryKey)
                    {
                        ...
                        if (primaryKeys.ContainsKey(field.ForeignKeyInfo.ToTable))
                        {
                            var p1 = new Point(leftPadding + (width + leftPadding) * count + width - 35, topPadding + countTable * height + custHeight + 8);
